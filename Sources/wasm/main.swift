...
func encodeVeif(data: JSTypedArray<UInt8>, width: Int, height: Int, bitrate: Int) async throws -> JSTypedArray<UInt8> {
    ...
    let jsArr = out.withUnsafeBytes { buf in
...
func rgbaToYCbCr(data: Data, width: Int, height: Int) -> YCbCrImage {
    var ycbcr = YCbCrImage(width: width, height: height, ratio: .ratio444)
    let bytesPerPixel = 4
    let bytesPerRow = (bytesPerPixel * width)
    
    data.withUnsafeBytes { rawPtr in
        guard let rawData = rawPtr.baseAddress?.assumingMemoryBound(to: UInt8.self) else { return }
...
func ycbcrToRGBA(img: YCbCrImage) -> Data {
    let width = img.width
    ...
    rawData.withUnsafeMutableBytes { buf in
        guard let dest = buf.baseAddress?.assumingMemoryBound(to: UInt8.self) else { return }
        ...
        for y in 0..<height {
            for x in 0..<width {
                ...
                let offset = ((y * bytesPerRow) + (x * bytesPerPixel))
                dest[offset + 0] = UInt8(clamping: r)
                dest[offset + 1] = UInt8(clamping: g)
                dest[offset + 2] = UInt8(clamping: b)
